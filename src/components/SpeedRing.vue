...
import {ETCSColors, Status, State} from '../etcsdefns';
...
function compute_path(start, end, radii) {
	const origin = calculate_pos(start, radii);
	let base = end;
	if (base > start + 180) {
		base = start + 180;
	}
	const endP = calculate_pos(base, radii);
	let path = "M " + origin.x + " " + origin.y + "A "+ radii + " " + radii + " 1 0 1 "+endP.x + " " + endP.y;
	if (end > start) {
		const secondEnd = calculate_pos(end, radii);
		path += "A 132.5 132.5 1 0 1" + secondEnd.x + " " + secondEnd.y;
	}
	return path
}
function hook(at) {
	const angular_setback = 6/117*180/Math.PI
	const endAngle = mapVelocity(at)
	const start = calculate_pos(endAngle - angular_setback, 127)
	const end = calculate_pos(endAngle, 127)
...
export default {
	...
	computed: {
		permit_path: function() {
			if (this.status == State.TSM) {
				return compute_path(-144, mapVelocity(Math.min(this.Vperm, this.Vtarget)), 132.5)
			}
			return compute_path(-144, mapVelocity(this.Vperm), 132.5);
		},
		target_path: function() {
			if (this.Vtarget < this.Vperm) return "";

...
import {ETCSColors, Mode, Status, State} from '../etcsdefns.ts';
export default {
	name: 'SpeedPointer',
	...
	computed: {
		...
		color: function() {
			if (this.mode == Mode.FS || this.mode == Mode.OS || this.mode == Mode.SR || this.mode == Mode.UN || this.mode == Mode.SH || this.mode == Mode.RV || this.mode == Mode.LS) {
